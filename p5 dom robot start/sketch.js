...
function setup() {
	...
	myRobot = new Robot("grey", false, "fin", 0);
    angleMode(DEGREES);
...
//    myRobot.colour = colourSelect.selected();//assign the chosen colour to our robor colour property
//    myRobot.rotation = rotationSlider.value();//assign robots rotation proerty to out rotation helper function value
//	
...
function Robot(colour, transmitting, name, rotation) {
    ...
    var nameText = createInput("fin");
    nameText.parent(controlsDiv);
    nameText.input(function(){
        self.name = this.value();
    })
    
    var colourSelect = createSelect();//creating a var for our method
   
    var colourOptions = ['grey', 'brown', 'red', 'green', 'yellow', 'purple'];
    for (var i = 0; i < colourOptions.length; i++)
        {
            colourSelect.option(colourOptions[i]);//fill up the var with options via loop        
        }
	...
	this.drawRobot = function() {
        ...
        rotate(this.rotation); //activate rotation
		//robots head
		fill(this.colour);
		strokeWeight(4);
